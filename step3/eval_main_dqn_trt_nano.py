...
ENGINE_PATH = '/home/jetson/catkin_ws/src/mavros-px4-vehicle/models/DQN_eval_learner.trt'
	...
	with jtop() as jetson:       
		for episode in range(1, args.episodes+1): #add one because python range() goes up to but not including the second value
			...
			while not done:
				...
				action = np.argmax(trt_outputs) #Get action from the output of TRT engine
				...
				print('Episode ' + str(episode) + ', episode steps = ' + str(episode_steps) + ', total steps = ' + str(total_steps))
