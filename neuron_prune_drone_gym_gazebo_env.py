...
register(
		...
		entry_point='neuron_prune_drone_gym_gazebo_env:DroneGymGazeboEnv',
...
class DroneGymGazeboEnv(gymnasium.Env):
	def __init__(self, render_mode="human"):
		...
		self.observation_space = spaces.Box(low=-1.0, high=8.0, shape=self.three_image_shape, dtype=np.float32)#11.0
		...
		self.desired_point.x = 7.0#10.0
		self.desired_point.y = self.start_point.y
		self.desired_point.z = self.start_point.z

		#Set workspace limits:
		self.work_space_x_max = 7.5#10.5
		self.work_space_x_min = -0.5
		self.work_space_y_max = 1.75#2.15
		self.work_space_y_min = -1.75#-2.15
	...
	def reset(self, options={}, seed=None):
		...
		if (self.drone.is_armed() == False):
			...
			rospy.loginfo("I = " + str(self.i) + ", self.start_point.y = " + str(self.start_point.y))
			self.desired_point.y = self.start_point.y
